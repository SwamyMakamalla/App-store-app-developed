...
class AppStore extends Component {
  state = {
    selectTabId: tabsList[0].tabId,
    searchInput: '',
  }

  searchAppWithInput = event => {
    const result = event.target.value
    this.setState({searchInput: result})
  }

  onTabSelect = tabId => {
    this.setState({selectTabId: tabId})
  }

  initializeApps = () => {
    const {selectTabId} = this.state
    const selectedTab = appsList.filter(
      eachApp => eachApp.category === selectTabId,
    )
    return selectedTab
  }
  ...
  render() {
    ...
    const filteredApps = this.initializeApps()

    const inputSearchResult = filteredApps.filter(app =>
      ...
      <div className="bg-container">
        ...
        <div className="user-container">
          <input
            ...
            onChange={this.searchAppWithInput}
          />
          <img
        ...
        <ul className="list-container">
          {tabsList.map(eachItem => (
            <TabItem
              ...
              activeClass={eachItem.tabId === selectTabId}
            />
          ))}
        </ul>
        <ul className="app-list-container">
          {inputSearchResult.map(eachItem => (
